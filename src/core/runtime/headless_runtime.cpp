    ...
    auto write_all_fields = [&](int export_index) -> bool
    {
        ...
        for (int k = 0; k < NZ; ++k)
        {
            ...
            for (int th = 0; th < NTH; ++th)
            {
                for (int i = 0; i < NR; ++i)
                {
                    ...
                    const float v_sample = static_cast<float>(v[i][th][k]);
        ...
        auto integrate_srh = [&](int i, int th, float depth_m) -> float
        {
            ...
            for (int k = 0; k < top_k; ++k)
            {
                ...
                const float v0 = static_cast<float>(v[i][th][k]) - storm_v_motion;
                const float u1 = static_cast<float>(u[i][th][k + 1]) - storm_u_motion;
                const float v1 = static_cast<float>(v[i][th][k + 1]) - storm_v_motion;
        ...
        auto ensure_surface_column_diagnostics = [&](int th)
        {
            ...
            for (int i = 0; i < NR; ++i)
            {
                ...
                const float v10_local = finite_or(sample_field_at_height(v, i, th, 10.0f), 0.0f);
                ...
                const float v6 = finite_or(sample_field_at_height(v, i, th, 6000.0f), v10_local);
        ...
        tranche_registry.push_back({"v10", "v10", "surface", [&](int th, std::vector<float>& out) {
            fill_slice_from_radius(out, [&](int i) { return sample_field_at_height(v, i, th, 10.0f); });
            return true;
        }});
            ...
            [&](int th, std::vector<float>& out) {
                fill_slice_from_radius(out, [&](int i) 
                {
                    ...
                    for (int k = 0; k < NZ; ++k)
                    {
                        ...
                        if (k <= vrot_top_k)
                        {
                            const float vrot = std::abs(static_cast<float>(v[i][th][k]));
        ...
        tranche_registry.push_back({"vrot", "vrot", "radar_synthetic", [&](int th, std::vector<float>& out) {
            fill_slice_from_radius(out, [&](int i) {
                ...
                {
                    const float sample = std::abs(static_cast<float>(v[i][th][k]));
                    if (std::isfinite(sample))
                    {
        ...
        // Determine if any derived or tranche fields are needed for this export.
        // On fast-only cadence ticks, only tight-tier core fields (u, v, w,
        // theta, rho, p, vorticity_*, etc.) are exported. ALL derived and
        // tranche fields are moderate or loose tier. Skipping the entire
        // derived computation loop on fast ticks eliminates the dominant
        // cost of the export step (~80% of export_s).
        bool need_derived = !derived_3d_bufs.empty();
        if (!need_derived)
        {
            for (const auto& df : exported_derived_fields)
            {
                if (rf.count(df.first)) { need_derived = true; break; }
            }
        }
        if (!need_derived)
        {
            for (const auto& tf : exported_tranche_fields)
            {
                if (rf.count(tf.field_id)) { need_derived = true; break; }
            }
        }
        ...
        for (int th = 0; th < NTH; ++th)
        {
            ...
            if (!need_derived)
            {
                continue; // Skip all derived computation for this theta
            }
            ...
            for (int k = 0; k < NZ; ++k)
            {
                for (int i = 0; i < NR; ++i)
                {
                    ...
                    const float v_center = static_cast<float>(v[i][th][k]);
                    ...
                    const float v_plus = static_cast<float>(v[i][th_plus][k]);
                    const float v_minus = static_cast<float>(v[i][th_minus][k]);
                    const float v_r_plus = static_cast<float>(v[(i + 1 < NR) ? i + 1 : i][th][k]);
                    const float v_r_minus = static_cast<float>(v[(i > 0) ? i - 1 : i][th][k]);
